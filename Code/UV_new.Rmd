...
```{r}
Nov3 <- DF[c(468:706), c("L", "a", "b", "treatment")]
```

The "before" group is listed as "NA" for treatment, so I want to change that 
```{r}
Nov3$treatment <- replace_na(Nov3$treatment, "control")
```
...
str(Nov3)
```
I want the treatment groups to be factors
```{r}
...
summary(model_L)
```
NL and UV treatments did not experience a significant change in luminosity over the 7-day trial period. The D treatment showed a significant increase in luminosity (p-value < 2e-16)

Let's make a nicer model:
```{r}
